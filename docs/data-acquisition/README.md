...
版本 CD v1.1，2026-09-16。本包响应“设计整个采集任务、为资料展示提供数据支持，并提供演示页面”的新需求，是既有 Release B 计划的增量。当前交付包括**具体来源接入设计、可执行的离线样例管线、JavDB 固定版本样本解析器、交互原型和验收记录**；自动联网执行、Jable 解析器、区域采集执行器和线上批次导入仍待准入与开发。
